...
export function calculatePriority(input: PriorityInput): PriorityResult {
  // Resolved reports get P4 (including 24h auto-closure)
  if (input.status === "resolved" || input.status === "auto_closed" || input.status === "presumed_resolved") {
    return { score: 0, level: "P4", ...PRIORITY_META.P4, factors: [input.status === "auto_closed" ? "Résolu (Auto-clôture 24h)" : "Résolu"] };
  }
