...
Deno.serve(async (req) => {
  ...
  try {
    const supabaseUrl=Deno.env.get("SUPABASE_URL")!; const serviceRoleKey=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!; const supabase=createClient(supabaseUrl,serviceRoleKey);
    const {data:reports,error}=await supabase.from("reports").select(`id,user_id,service_type,commune,quartier,created_at,reminder_count,last_reminder_at,report_category,status,j3_author_notified,j7_author_notified,description`).in("status",["active","chronic"]).or("report_category.eq.outage,and(report_category.eq.infrastructure,service_type.in.(electricity,water))");
    if(error) throw error; if(!reports||reports.length===0) return new Response(JSON.stringify({processed:0}),{headers:{...corsHeaders,"Content-Type":"application/json"}});
    const now=Date.now(),APP_URL="https://signa.ci"; let notificationsInserted=0,archived=0,escalated=0,expired14d=0,j3Sent=0,j7Sent=0;
    for(const report of reports){
      ...
      else if(ageHours>=10&&ageHours<24&&lastReminderMinutes>=175){shouldRemind=true;reminderTitle=isInfra?`🟠 ${ageDisplay} — problème ${operatorName} toujours présent ?`:`🟠 ${ageDisplay} de coupure — toujours actif ?`;reminderMessage=`${serviceLabel} — ${report.commune}, ${report.quartier} · Signalé il y a ${ageDisplay}. ${verifyVerb} → ${detailUrl}`;}
      else if(ageHours>=6&&ageHours<10&&report.reminder_count<2){shouldRemind=true;reminderTitle=isInfra?`🟠 Problème ${operatorName} — toujours présent ?`:"🟠 Toujours sans service ?";reminderMessage=`${serviceLabel} — ${report.commune}, ${report.quartier} · Signalé il y a ~6h. ${verifyVerb} → ${detailUrl}`;}
      else if(ageHours>=3&&ageHours<6&&report.reminder_count<1){shouldRemind=true;reminderTitle=isInfra?`⏰ Problème ${operatorName} — résolu ?`:"⏰ Coupure toujours active ?";reminderMessage=`${serviceLabel} — ${report.commune}, ${report.quartier} · Signalé il y a ~3h. ${verifyVerb} → ${detailUrl}`;}
      if(shouldRemind){await supabase.from("notifications").insert({user_id:report.user_id,report_id:report.id,title:reminderTitle,message:reminderMessage});await supabase.from("reports").update({last_reminder_at:new Date().toISOString(),reminder_count:report.reminder_count+1}).eq("id",report.id);notificationsInserted++;}
    }
    return new Response(JSON.stringify({processed:reports.length,notifications:notificationsInserted,archived,escalated,chronicified:expired14d,j3_sent:j3Sent,j7_sent:j7Sent}),{headers:{...corsHeaders,"Content-Type":"application/json"}});
