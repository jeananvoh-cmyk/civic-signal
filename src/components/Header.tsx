      ...
      <header className={`sticky top-0 z-50 border-b border-border/60 bg-background/90 backdrop-blur-md supports-[backdrop-filter]:bg-background/80 transition-shadow duration-200 ${scrolled ? "shadow-[0_2px_12px_hsl(var(--foreground)/0.06)]" : ""}`}>
        <div className="container flex h-[3.75rem] items-center justify-between gap-4">
          ...
          <Link to="/" className="flex items-center gap-2.5 shrink-0 group">
            <div className="relative flex h-8 w-8 items-center justify-center rounded-[10px] bg-primary shadow-[0_1px_3px_hsl(var(--primary)/0.4)] group-hover:shadow-[0_2px_8px_hsl(var(--primary)/0.5)] transition-shadow">
              <svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-[18px] w-[18px]">
                <circle cx="10" cy="8" r="3.5" fill="white" />
                <circle cx="10" cy="8" r="5.5" stroke="white" strokeOpacity="0.35" strokeWidth="1.2" />
                <path d="M10 13.5 L10 19" stroke="white" strokeWidth="1.8" strokeLinecap="round" />
                <path d="M7 17.5 L10 19 L13 17.5" stroke="white" strokeOpacity="0.5" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <div className="flex flex-col leading-none">
              <span className="text-[15px] font-bold tracking-[-0.025em] text-foreground">
                SIGNA<span className="text-primary">·CI</span>
              </span>
              <span className="text-[9px] font-semibold tracking-[0.12em] text-muted-foreground/70 uppercase mt-px">
                Côte d'Ivoire
              </span>
            </div>
          </Link>
          ...
          {mobileOpen && (
            ...
            >
              <div className="container py-3 space-y-0.5">
                ...
                <div className="flex items-center justify-between px-4 py-3 rounded-lg hover:bg-muted/60 transition-colors cursor-pointer" onClick={toggleTheme}>
                  <span className="text-[14px] font-medium text-foreground/70">Thème d'affichage</span>
                  <div className="flex items-center gap-2 text-muted-foreground">{themeIcon}<span className="text-[13px] capitalize">{theme === "system" ? "Système" : theme === "dark" ? "Sombre" : "Clair"}</span></div>
                </div>
              </div>
            </motion.nav>
          )}
